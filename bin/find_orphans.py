...
def populate(host=None):
    ...
    for f in list(unfiltered):
        if (f.endswith('.en.srt') or f.endswith('.nfo')):
            supportfiles.append(f)
            unfiltered.remove(f)
    
    unusedfiles = []        
    for f in list(supportfiles):
        if f.endswith('.en.srt'):
            v = f[:-7]
        elif f.endswith('.nfo'):
            v = f[:-4]
        for g in list(unfiltered):
            if not g.startswith(v):
                unusedfiles.add(f)
                supportfiles.remove(f)

    unfiltered += unusedfiles

    for f in list(unfiltered):
        if (f.endswith('.mp4') or f.endswith('.mkv') or f.endswith('.avi') or f.endswith('.flv')):
            nonnative.append(f)
            unfiltered.remove(f)
        elif (f.endswith('.mpg') or f.endswith('.nuv') or f.endswith('.ts')):
            orphvids.append(f)
            unfiltered.remove(f)
    ...
    return (recs, zerorecs, orphvids, orphimgs, dbbackup, nonnative, supportfiles, unfiltered)
...
def main(host=None):
   while True:
        recs, zerorecs, orphvids, orphimgs, dbbackup, nonnative, supportfiles, unfiltered = populate(host)
        ...
        if len(orphvids):
            printfiles("Orphaned recording files", orphvids)
        if len(nonnative):
            printfiles("Orphaned downloaded videos", nonnative)
        if len(orphimgs):
            printfiles("Orphaned thumbnails", orphimgs)
        if len(dbbackup):
        ...
        if len(nonnative):
            opts.append(['Delete orphaned downloaded videos', delete_files, nonnative + supportfiles])
        if len(orphimgs):
            opts.append(['Delete orphaned thumbnails', delete_files, orphimgs])
        if len(unfiltered):
            opts.append(['Delete other files', delete_files, unfiltered])
        opts.append(['Refresh list', None, None])
        opts.append(['Exit',sys.exit, 0])
